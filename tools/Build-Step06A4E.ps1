...
$build=Join-Path $repo 'build\step06a4e-build'
$tests=Join-Path $repo 'build\step06a4e-tests'
$base=Join-Path $repo 'build\step06a4c-player\runtime'
$stage=Join-Path $repo 'build\step06a4e-player\runtime'
...
if($running.Count){throw 'Close the Step06A4E test player before replacing its executable.'}
...
@('STEP06A4E_RUNTIME=1','BASE=Step06A4C','CONFIDENCE_POLICY=Step06A3 exact S10.5','GLOBAL_MEDIAN=parallel_histogram','STREAMLINE_FAILED_DEVICE=native_fallback',('EXE_SHA256='+(Get-FileHash -LiteralPath $exe).Hash)) |
    Set-Content -LiteralPath (Join-Path $stage 'step06a4e-runtime.txt') -Encoding utf8
if((Get-FileHash -LiteralPath $release).Hash -ne $protectedHash){throw 'Official Release changed unexpectedly'}
Write-Host "STEP06A4E_BUILD=PASS runtime=$exe"
