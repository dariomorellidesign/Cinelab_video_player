...
static constexpr UINT IDM_NVOF_CONTRAST_OFF = 390;
static constexpr UINT IDM_NVOF_CONTRAST_MILD = 391;
static constexpr UINT IDM_NVOF_CONTRAST_MEDIUM = 392;
static constexpr UINT IDM_NVOF_CONTRAST_STRONG = 393; // STEP 06A1 NVOF analysis-only contrast prefilter
static constexpr UINT IDM_NVOF_SHADOW_OFF = 394;
static constexpr UINT IDM_NVOF_SHADOW_LOW = 395;
static constexpr UINT IDM_NVOF_SHADOW_MEDIUM = 396; // STEP 06A2 shadow-noise-aware NVOF preprocessing
...
struct AppOptions {
    ...
    // Development diagnostic: decouple the decoder's size from the selected DLSS mode.
    // This is intentionally command-line only; it isolates source prefiltering from
    // DLSS model behavior without adding a consumer control to the player UI.
    enum class DecodeResolution { Auto, Native, Third } decodeResolution=DecodeResolution::Auto;
    bool forceCurrentColor=false;
    bool gpuMotion=true;
...
static AppOptions ParseArgs() {
    ...
    for(int i=1;i<argc;++i) {
        ...
        } else if(a==L"--motion-pipeline" && i+1<argc){o.gpuMotion=std::wstring(argv[++i])!=L"reference";
        } else if(a==L"--decode-resolution" && i+1<argc) {
            std::wstring v=argv[++i];std::transform(v.begin(),v.end(),v.begin(),::towlower);
            if(v==L"native") o.decodeResolution=AppOptions::DecodeResolution::Native;
            else if(v==L"third") o.decodeResolution=AppOptions::DecodeResolution::Third;
        } else if(a==L"--dlss-history" && i+1<argc) {
            std::wstring v=argv[++i];std::transform(v.begin(),v.end(),v.begin(),::towlower);
            o.forceCurrentColor=(v==L"current");
    ...
    if(o.gpuMotion)o.depthSource=D3D12Renderer::DepthSource::Flat;
...
class PlayerApp {
    float m_selectedDepthAgeMs=-1.0f;
        ...
        if(frameProcessOk) {
            ...
            if(fpsElapsed>=0.75){
                ...
                if(m_renderer){
                    ...
                    LOG("[Pipeline] quality="<<QualityNameA(m_activeQuality)<<" nvofMode="<<(m_opticalFlow?m_opticalFlow->PerfName():"OFF")<<" grid="<<(m_opticalFlow?m_opticalFlow->GridSize():0)<<" nvofMs="<<ofStats.lastTotalMs<<" nvofPreMs="<<ofStats.lastPreprocessMs<<" nvofPreEmaMs="<<ofStats.emaPreprocessMs<<" nvofUploadMs="<<ofStats.lastUploadMs<<" nvofExecuteMs="<<ofStats.lastExecuteMs<<" nvofDownloadMs="<<ofStats.lastDownloadMs<<" nvofConvertMs="<<ofStats.lastConvertMs<<" guidesMs="<<m_lastGuidesMs<<" guideFast="<<(m_lastGuideHardwareFastPath?1:0)<<" legacyFlow="<<(m_lastGuideLegacyFlow?1:0)<<" rendererMs="<<m_lastRendererMs<<" otherMs="<<pipelineOtherMs<<" frameMs="<<m_lastFrameProcessMs<<" aiStaleRecoveries="<<aiStats.staleRecoveries);
                }
            }
        }
...
private:
    ...
    void LoadVideoSettings(){
        ...
        // STEP 06A3: old analysis preprocessors are retired and hidden.
        m_nvofAnalysisContrast=0;m_nvofShadowFilter=0;
        _wputenv_s(L"DMP_NVOF_ANALYSIS_CONTRAST",L"off");
        _wputenv_s(L"DMP_NVOF_SHADOW_FILTER",L"off");
    ...
    void SaveVideoSettings()const{
        ...
        WriteIniFloat(L"OpticalFlow",L"AnalysisContrast",float(m_nvofAnalysisContrast));
        WriteIniFloat(L"OpticalFlow",L"AnalysisContrastSchema",2.0f);
        WriteIniFloat(L"OpticalFlow",L"ShadowFilter",float(m_nvofShadowFilter));
        const auto path=SettingsPath();
    ...
    UINT CurrentNvofContrastMenuId()const{
        return IDM_NVOF_CONTRAST_OFF+UINT(std::clamp(m_nvofAnalysisContrast,0,3));
    }
    static const wchar_t* NvofContrastEnvValue(int level){
        switch(std::clamp(level,0,3)){case 1:return L"mild";case 2:return L"medium";case 3:return L"strong";default:return L"off";}
    }
    UINT CurrentNvofShadowMenuId()const{
        return IDM_NVOF_SHADOW_OFF+UINT(std::clamp(m_nvofShadowFilter,0,2));
    }
    static const wchar_t* NvofShadowEnvValue(int level){
        switch(std::clamp(level,0,2)){case 1:return L"low";case 2:return L"medium";default:return L"off";}
        ...
        if(m_nvofContrastMenu)CheckMenuRadioItem(m_nvofContrastMenu,IDM_NVOF_CONTRAST_OFF,IDM_NVOF_CONTRAST_STRONG,CurrentNvofContrastMenuId(),MF_BYCOMMAND);
        if(m_nvofShadowMenu)CheckMenuRadioItem(m_nvofShadowMenu,IDM_NVOF_SHADOW_OFF,IDM_NVOF_SHADOW_MEDIUM,CurrentNvofShadowMenuId(),MF_BYCOMMAND);
    ...
    void SetNvofAnalysisContrast(int level){
        if(m_opt.gpuMotion)return;
        const int next=std::clamp(level,0,3);if(m_nvofAnalysisContrast==next)return;
        m_nvofAnalysisContrast=next;const wchar_t* value=NvofContrastEnvValue(next);_wputenv_s(L"DMP_NVOF_ANALYSIS_CONTRAST",value);
        const char* label=next==1?"mild":(next==2?"medium":(next==3?"strong":"off"));
        SaveVideoSettings();LOG("[NVOF UI] analysisContrast="<<label<<" (NVOF input only)");UpdateNvofMenuChecks();ReloadForNvofChange();
    }
    void SetNvofShadowFilter(int level){
        if(m_opt.gpuMotion)return;
        const int next=std::clamp(level,0,2);if(m_nvofShadowFilter==next)return;
        m_nvofShadowFilter=next;const wchar_t* value=NvofShadowEnvValue(next);_wputenv_s(L"DMP_NVOF_SHADOW_FILTER",value);
        const char* label=next==1?"low":(next==2?"medium":"off");
        SaveVideoSettings();LOG("[NVOF UI] shadowFilter="<<label<<" (NVOF input only)");UpdateNvofMenuChecks();ReloadForNvofChange();
    ...
    void SetDepthSource(D3D12Renderer::DepthSource source){
        if(m_opt.gpuMotion){LOG("[GPU MV] depth source fixed: no depth computation, mask zero");return;}
        if(m_opt.depthSource==source)return;
        m_opt.depthSource=source;
        m_guides.Reset();m_guideReset=true;
        if(source!=D3D12Renderer::DepthSource::AISynthetic)m_aiDepthWorker.reset();
    ...
    HMENU CreateMenuBar() {
        ...
        HMENU bar=CreateMenu(),file=CreatePopupMenu(),play=CreatePopupMenu(),video=CreatePopupMenu(),dlss=CreatePopupMenu(),quality=CreatePopupMenu(),nvof=CreatePopupMenu(),nvofPerf=CreatePopupMenu(),nvofGrid=CreatePopupMenu(),depthSource=CreatePopupMenu(),frameGen=CreatePopupMenu();m_nvofPerfMenu=nvofPerf;m_nvofGridMenu=nvofGrid;m_nvofContrastMenu=nullptr;m_nvofShadowMenu=nullptr;m_depthSourceMenu=depthSource;
        ...
        add(video,IDM_VIEW_FINAL,L"menu.final"); add(video,IDM_VIEW_INPUT,L"menu.input"); add(video,IDM_VIEW_MV,L"menu.mv"); AppendMenuW(video,MF_SEPARATOR,0,nullptr); add(video,IDM_FULLSCREEN,L"menu.fullscreen");
        ...
        add(dlss,IDM_DLSS,L"menu.dlss_toggle"); add(dlss,IDM_REHOOK,L"menu.rehook"); std::wstring qualityName=T(L"menu.quality"); AppendMenuW(dlss,MF_POPUP,reinterpret_cast<UINT_PTR>(quality),qualityName.c_str()); AppendMenuW(dlss,MF_POPUP,reinterpret_cast<UINT_PTR>(nvof),L"Optical Flow (NVOF)");
        ...
        if(m_opt.gpuMotion){
            for(UINT id=IDM_DEPTH_SOURCE_LEGACY;id<=IDM_DEPTH_SOURCE_AI;++id)EnableMenuItem(m_depthSourceMenu,id,MF_BYCOMMAND|MF_GRAYED);
            for(UINT id=IDM_NVOF_CONTRAST_OFF;id<=IDM_NVOF_CONTRAST_STRONG;++id)EnableMenuItem(m_nvofContrastMenu,id,MF_BYCOMMAND|MF_GRAYED);
            for(UINT id=IDM_NVOF_SHADOW_OFF;id<=IDM_NVOF_SHADOW_MEDIUM;++id)EnableMenuItem(m_nvofShadowMenu,id,MF_BYCOMMAND|MF_GRAYED);
            EnableMenuItem(bar,IDM_TEMPORAL_MASK_BYPASS,MF_BYCOMMAND|MF_GRAYED);
        }
        return bar;
    }
    ...
    void StartAIDepth(){
        ...
        m_aiDepthWorker=std::move(worker);LOG("[AI Depth] selected producer started; one temporal snapshot for mask, DLSS and FG; missing depth uses explicit FLAT fallback; delayed valid AI is held with visible age.");
    }
    bool Load(const std::wstring& path) {
        ...
        auto [decodeW,decodeH]=RecommendedDecodeSize(m_decoder.NativeWidth(),m_decoder.NativeHeight(),ow,oh,m_activeQuality);
        if(m_opt.decodeResolution==AppOptions::DecodeResolution::Native){decodeW=m_decoder.NativeWidth();decodeH=m_decoder.NativeHeight();LOG("[Decode Diagnostic] forcing native decode; DLSS mode remains "<<QualityNameA(m_activeQuality));}
        else if(m_opt.decodeResolution==AppOptions::DecodeResolution::Third){decodeW=std::max(2u,uint32_t(std::lround(double(ow)/3.0))&~1u);decodeH=std::max(2u,uint32_t(std::lround(double(oh)/3.0))&~1u);LOG("[Decode Diagnostic] forcing one-third-output decode; DLSS mode remains "<<QualityNameA(m_activeQuality));}
    ...
    void Unload() {
        ...
        m_seekPending=false;m_seeking=false;m_audio.Stop(); m_aiDepthWorker.reset();m_aiDepthLatest={};m_aiDepthSequence=0;m_aiDepthTemporalWorker.Reset(); if(m_renderer)m_renderer->WaitGPU();m_opticalFlow.reset(); if(m_renderer){m_renderer->WaitGPU();m_renderer.reset();} m_decoder.Close();m_guides.Reset();m_haveNext=false;m_next=VideoFrame{};m_loaded=false;m_playing=false;m_currentSec=0;m_lastRenderedTs=-1;m_path.clear();
        if(m_viewport)ShowWindow(m_viewport,SW_HIDE); UpdateTitle(); if(m_hwnd)InvalidateRect(m_hwnd,nullptr,TRUE);
    }

    bool RenderGpuMotionFrame(const VideoFrame& f,bool reset){
        if(!m_renderer||!m_opticalFlow){LOG("[GPU MV] NVOF unavailable; stopping instead of silently using CPU flow");return false;}
        const bool cut=m_sparseSceneCut.Update(f.bgra.data(),m_decoder.Width(),m_decoder.Height(),reset||f.discontinuity);
        if(cut){reset=true;LOG("[Scene Cut GPU pipeline] sparse decoder samples: reset NVOF/DLSS/FG ts="<<f.timestamp100ns);}
        GpuOpticalFlowFrame flow;
        if(!m_opticalFlow->GenerateGpu(f.bgra.data(),f.bgra.size(),reset||f.discontinuity,m_renderer->CompletionFence(),m_renderer->CompletionValue(),flow))return false;
        float ms=float(1000.0/std::max(1.0,m_decoder.FrameRate()));
        if(m_lastRenderedTs>=0&&f.timestamp100ns>m_lastRenderedTs)ms=float(double(f.timestamp100ns-m_lastRenderedTs)*1e-4);
        const auto start=Clock::now();
        const bool ok=m_renderer->RenderFrame(nullptr,0,nullptr,0,0,0,m_dlssReset||reset||!flow.valid,ms,nullptr,0,0,0,&flow);
        m_lastRendererMs=std::chrono::duration<double,std::milli>(Clock::now()-start).count();
        m_lastGuidesMs=0;m_lastGuideHardwareFastPath=true;m_lastGuideLegacyFlow=false;
        if((m_renderer->FramesPresented()%120u)==0u)LOG("[MV Pair] previousPresentedTs="<<m_lastRenderedTs<<" currentPresentedTs="<<f.timestamp100ns<<" pairValid="<<flow.valid<<" deltaMs="<<ms<<" rendererAccepted="<<ok);
        if(ok)m_lastRenderedTs=f.timestamp100ns;
        return ok;
    }

    bool RenderVideoFrame(const VideoFrame& f,bool resetGuide) {
        if(m_opt.gpuMotion)return RenderGpuMotionFrame(f,resetGuide);
        ...
        const bool useAI = m_opt.depthSource==D3D12Renderer::DepthSource::AISynthetic;
        if(useAI && !m_aiDepthWorker && m_renderer->FramesPresented()>=90)StartAIDepth();
        ...
        // One immutable AI snapshot for all depth consumers in this frame.
        ...
        if(useAI&&aiMaskSnapshot&&aiMaskSnapshot->valid&&aiMaskSnapshot->preview01.size()==size_t(AIDepthTemporalStabilizer::DepthW)*AIDepthTemporalStabilizer::DepthH){
            ...
            aiMaskDepth.ageMs=float(ageMs);aiMaskDepth.ageFrames=float(ageFrames);aiMaskDepth.valid=true;
            aiMaskDepthPtr=&aiMaskDepth;
        }
        m_guides.SetDepthMode(m_opt.depthSource==D3D12Renderer::DepthSource::Legacy ? TemporalGuideGenerator::DepthMode::Estimated : (useAI && aiMaskDepth.valid ? TemporalGuideGenerator::DepthMode::AI : TemporalGuideGenerator::DepthMode::Flat));
        m_selectedDepthAgeMs=useAI?aiMaskDepth.ageMs:-1.0f;
        ...
        static uint64_t maskDepthDiag=0;if((++maskDepthDiag%120u)==0u)LOG("[Mask Depth] effective="<<(g.maskUsedAIDepth?"AI":(m_opt.depthSource==D3D12Renderer::DepthSource::Legacy?"LEGACY":"FLAT"))<<" aiAvailable="<<(g.maskAIDepthAvailable?1:0)<<" ageMs="<<g.maskDepthAgeMs<<" ageFrames="<<g.maskDepthAgeFrames);
        ...
        if(sceneCutReset){
            if(m_aiDepthWorker)m_aiDepthWorker->Reset();
            m_aiDepthLatest={};m_aiDepthSequence=0;newAIDepth=false;
        ...
        if(useAI)(void)m_aiDepthTemporalWorker.Submit(f.timestamp100ns,m_lastRenderedTs,m_decoder.Width(),m_decoder.Height(),aiMotionPtr,aiMeasurementPtr);
        // Reuse the exact snapshot supplied to the mask; never fetch a newer one mid-frame.
        const auto aiTemporal=aiMaskSnapshot;
        const bool aiTemporalValid=useAI && g.maskUsedAIDepth && !sceneCutReset;
        if(!aiTemporalValid && m_renderer->GetEffectiveDepthSource()==D3D12Renderer::DepthSource::AISynthetic)m_renderer->ResetAIDepthDebug();
        ...
        bool ok=m_renderer->RenderFrame(f.bgra.data(),f.bgra.size(),g.guideGridRGBA32F.data(),g.guideGridRGBA32F.size()*sizeof(float),g.gridW,g.gridH,r,ms,aiPreview,aiBytes,AIDepthTemporalStabilizer::DepthW,AIDepthTemporalStabilizer::DepthH,nullptr);
    ...
    void UpdateTitle(){
        ...
        const std::wstring fileName=std::filesystem::path(m_path).filename().wstring();
        std::wstringstream s;s<<fileName<<L"  |  "<<TimeText(Position())<<L"  |  SR ";
    ...
    int ButtonWidth(int idx)const{static const int widths[]={56,42,46,42,42,84,74,108,60,54};return (idx>=0&&idx<10)?widths[idx]:0;}
    RECT ButtonRect(int idx)const{RECT c=ControlClientRect();int x=12;for(int i=0;i<idx&&i<10;++i)x+=ButtonWidth(i)+5;return RECT{x,9,x+ButtonWidth(idx),45};}
    ...
    int HitTestButton(int x,int y)const{for(int i=0;i<10;++i)if(PtIn(ButtonRect(i),x,y))return i;if(PtIn(MuteRect(),x,y))return 100;return -1;}
    RECT HoverRect(int id)const{if(id>=0&&id<10)return ButtonRect(id);if(id==100)return MuteRect();return RECT{};}
    ...
    void CreateDebugTooltips(){
        ...
        add(1001,5,L"Super Resolution\nTurns the DLSS carrier path on or off.");
        add(1002,6,L"Frame Generation\nCycles Off, 2x, 3x and the available multipliers.");
        add(1003,7,L"Settings\nImage, Super Resolution, Frame Generation, audio and subtitles.");
        add(1004,9,L"Help\nA brief guide to Super Resolution, Frame Generation, NVOF and RenoDX NR.");
    }
    void UpdateTooltipRects(){
        if(!m_tooltipWnd||!m_controlsWnd)return;auto upd=[&](UINT_PTR id,int button){TTTOOLINFOW ti{sizeof(ti)};ti.uFlags=TTF_SUBCLASS;ti.hwnd=m_controlsWnd;ti.uId=id;ti.rect=ButtonRect(button);SendMessageW(m_tooltipWnd,TTM_NEWTOOLRECTW,0,reinterpret_cast<LPARAM>(&ti));};upd(1001,5);upd(1002,6);upd(1003,7);upd(1004,9);
    }
    ...
    void ControlsPaint(){
        ...
        const std::wstring dlssButton=dlssRequested?L"SR ON":L"SR OFF";
        ...
        DrawButton(mem,ButtonRect(5),dlssButton,dlssRequested,m_hoverButton==5);
        DrawButton(mem,ButtonRect(6),fgButton,m_frameGenerationEnabled&&fgAvailable,m_hoverButton==6);
        DrawButton(mem,ButtonRect(7),L"Settings",false,m_hoverButton==7);
        DrawButton(mem,ButtonRect(8),L"Full",m_fullscreen,m_hoverButton==8);
        DrawButton(mem,ButtonRect(9),L"?",false,m_hoverButton==9);
        ...
        DrawButton(mem,MuteRect(),m_muted?L"\U0001F507":L"\U0001F50A",m_muted,m_hoverButton==100);
        ...
        std::wstringstream st;if(m_seeking||m_seekPending)st<<T(L"status.seeking")<<L"  |  ";st<<L"SR "<<(m_renderer&&m_renderer->DLSSEnabled()?QualityNameW(m_activeQuality):L"OFF")<<L"  |  DLAA L  |  "<<m_decoder.NativeWidth()<<L"x"<<m_decoder.NativeHeight()<<L" -> "<<m_renderer->OutputW()<<L"x"<<m_renderer->OutputH();if(m_droppedFrames)st<<L"  |  drops "<<m_droppedFrames;std::wstring status=st.str();RECT sr{145,c.bottom-48,std::max<LONG>(146,c.right-445),c.bottom-28};DrawTextW(mem,status.c_str(),-1,&sr,DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
        ...
        wchar_t fpsBuf[224]{};
        swprintf_s(fpsBuf,L"Video %.2f FPS | Display %.1f FPS",sourceFps,m_displayFps);
    ...
    void ShowSettingsMenu(){
        HMENU root=CreatePopupMenu(),quality=CreatePopupMenu(),fg=CreatePopupMenu(),audio=CreatePopupMenu(),subs=CreatePopupMenu(),developer=CreatePopupMenu();
        auto item=[](HMENU m,UINT id,const wchar_t* label,bool checked=false,bool enabled=true){AppendMenuW(m,MF_STRING|(checked?MF_CHECKED:0)|(enabled?0:MF_GRAYED),id,label);};
        item(quality,IDM_QUALITY_AUTO,L"Auto",!m_opt.qualityExplicit);item(quality,IDM_QUALITY_DLAA,L"DLAA (recommended)",m_opt.qualityExplicit&&m_opt.quality==NVSDK_NGX_PerfQuality_Value_DLAA);item(quality,IDM_QUALITY_QUALITY,L"Quality");item(quality,IDM_QUALITY_BALANCED,L"Balanced");item(quality,IDM_QUALITY_PERFORMANCE,L"Performance");item(quality,IDM_QUALITY_ULTRAPERF,L"Ultra Performance");
        item(fg,IDM_FRAMEGEN_OFF,L"Off",!m_frameGenerationEnabled);const uint32_t fgCap=m_vsyncEnabled?VSyncFrameGenerationCap():6u;const bool fgReady=m_renderer&&m_renderer->FrameGenerationAvailable();for(UINT i=2;i<=6;++i)item(fg,IDM_FRAMEGEN_2X+i-2,(L"DLSS-G "+std::to_wstring(i)+L"x").c_str(),m_frameGenerationEnabled&&m_frameGenerationMultiplier==i,fgReady&&i<=fgCap);
        if(m_mediaTracks.AudioTracks().empty())item(audio,0,L"No audio tracks",false,false);else for(size_t i=0;i<m_mediaTracks.AudioTracks().size()&&i<90;++i){const auto&t=m_mediaTracks.AudioTracks()[i];item(audio,IDM_AUDIO_TRACK_BASE+UINT(i),MediaTrackCatalog::MenuLabel(t).c_str(),t.streamIndex==m_selectedAudioStream);}
        item(subs,IDM_SUBTITLE_OFF,L"Off",m_selectedSubtitleStream<0);for(size_t i=0;i<m_mediaTracks.SubtitleTracks().size()&&i<90;++i){const auto&t=m_mediaTracks.SubtitleTracks()[i];item(subs,IDM_SUBTITLE_TRACK_BASE+UINT(i),MediaTrackCatalog::MenuLabel(t).c_str(),t.streamIndex==m_selectedSubtitleStream,t.textSubtitleSupported);}
        item(developer,IDM_VIEW_FINAL,L"Final output");item(developer,IDM_VIEW_INPUT,L"SR input");item(developer,IDM_VIEW_MV,L"Motion vectors");AppendMenuW(developer,MF_SEPARATOR,0,nullptr);item(developer,IDM_SPLIT_SCREEN,L"Split comparison",m_splitScreen);item(developer,IDM_NVOF_PERF_SLOW,L"NVOF: Slow");item(developer,IDM_NVOF_PERF_MEDIUM,L"NVOF: Medium");item(developer,IDM_NVOF_PERF_FAST,L"NVOF: Fast");
        item(root,IDM_DLSS,m_renderer&&m_renderer->DLSSRequested()?L"Super Resolution: On":L"Super Resolution: Off",m_renderer&&m_renderer->DLSSRequested());AppendMenuW(root,MF_POPUP,reinterpret_cast<UINT_PTR>(quality),L"Super Resolution quality");AppendMenuW(root,MF_POPUP,reinterpret_cast<UINT_PTR>(fg),L"Frame Generation");item(root,IDM_VSYNC,m_vsyncEnabled?L"V-Sync: On":L"V-Sync: Off",m_vsyncEnabled);AppendMenuW(root,MF_SEPARATOR,0,nullptr);item(root,IDM_VIDEO_ADJUSTMENTS,L"Image adjustments...");item(root,m_fill?IDM_ASPECT_FIT:IDM_ASPECT_FILL,m_fill?L"Fit image":L"Fill screen");AppendMenuW(root,MF_SEPARATOR,0,nullptr);AppendMenuW(root,MF_POPUP,reinterpret_cast<UINT_PTR>(audio),L"Audio track");AppendMenuW(root,MF_POPUP,reinterpret_cast<UINT_PTR>(subs),L"Subtitles");AppendMenuW(root,MF_SEPARATOR,0,nullptr);AppendMenuW(root,MF_POPUP,reinterpret_cast<UINT_PTR>(developer),L"Developer tools");
        RECT r=ButtonRect(7);POINT p{r.left,r.bottom};ClientToScreen(m_controlsWnd,&p);TrackPopupMenu(root,TPM_LEFTALIGN|TPM_TOPALIGN|TPM_RIGHTBUTTON,p.x,p.y,0,m_hwnd,nullptr);DestroyMenu(root);
    }
    void ShowHelp(){
        const wchar_t* guide=L"QUICK GUIDE\n\n"
            L"Super Resolution\nDLAA uses native resolution with preset L and is recommended for film playback. Quality, Balanced, Performance and Ultra Performance lower the internal resolution to create more performance headroom.\n\n"
    ...
    void ControlsMouseDown(int x,int y){
        ...
        const int b=HitTestButton(x,y);switch(b){case 0:OpenFromDialog();break;case 1:RequestSeek(Position()-10);break;case 2:TogglePause();break;case 3:StopPlayback();break;case 4:RequestSeek(Position()+10);break;case 5:ToggleDLSS();break;case 6:CycleFrameGeneration();break;case 7:ShowSettingsMenu();break;case 8:ToggleFullscreen();break;case 9:ShowHelp();break;}
    }
    ...
    void ToggleDepthMode(){SetDepthSource(m_opt.depthSource==D3D12Renderer::DepthSource::Flat?D3D12Renderer::DepthSource::Legacy:D3D12Renderer::DepthSource::Flat);}
    ...
    void CycleBiasMaskMode(){if(!m_renderer)return;const auto mode=m_renderer->CycleBiasMaskMode();LOG("[DLSS Mask] mode="<<(mode==D3D12Renderer::BiasMaskMode::Auto?"AUTO":(mode==D3D12Renderer::BiasMaskMode::Off?"OFF":"FORCE_CURRENT"))<<"; history preserved");InvalidateControls();}
    ...
    void HandleCommand(UINT id){
        ...
        case IDM_NVOF_GRID_AUTO:SetNvofGrid(L"auto");break;case IDM_NVOF_GRID_1:SetNvofGrid(L"1");break;case IDM_NVOF_GRID_2:SetNvofGrid(L"2");break;case IDM_NVOF_GRID_4:SetNvofGrid(L"4");break;case IDM_NVOF_CONTRAST_OFF:SetNvofAnalysisContrast(0);break;case IDM_NVOF_CONTRAST_MILD:SetNvofAnalysisContrast(1);break;case IDM_NVOF_CONTRAST_MEDIUM:SetNvofAnalysisContrast(2);break;case IDM_NVOF_CONTRAST_STRONG:SetNvofAnalysisContrast(3);break;case IDM_NVOF_SHADOW_OFF:SetNvofShadowFilter(0);break;case IDM_NVOF_SHADOW_LOW:SetNvofShadowFilter(1);break;case IDM_NVOF_SHADOW_MEDIUM:SetNvofShadowFilter(2);break;        case IDM_QUALITY_AUTO:SetQualityMode(true,NVSDK_NGX_PerfQuality_Value_MaxQuality);break;case IDM_QUALITY_QUALITY:SetQualityMode(false,NVSDK_NGX_PerfQuality_Value_MaxQuality);break;case IDM_QUALITY_BALANCED:SetQualityMode(false,NVSDK_NGX_PerfQuality_Value_Balanced);break;case IDM_QUALITY_PERFORMANCE:SetQualityMode(false,NVSDK_NGX_PerfQuality_Value_MaxPerf);break;case IDM_QUALITY_ULTRAPERF:SetQualityMode(false,NVSDK_NGX_PerfQuality_Value_UltraPerformance);break;case IDM_QUALITY_DLAA:SetQualityMode(false,NVSDK_NGX_PerfQuality_Value_DLAA);break;
    ...
    SparseSceneCut m_sparseSceneCut;
